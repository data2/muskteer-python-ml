...
def parse_content(response, id):

        try:
            baseinfo_json = response[0]
            res = {'secCode': id, 'bossAssist': baseinfo_json['F004V'], 'financeManager': baseinfo_json['F003V'],
                   'bigBoss': baseinfo_json['F001V'], 'boss': baseinfo_json['F005V'], 'bigAdmin': baseinfo_json['F002V']}
            print(res)
            with open('../data/bossinfo.txt', "a") as file:
                file.write(str(res) + "\n")
            file.close()
        except Exception as e:
            print(e)
