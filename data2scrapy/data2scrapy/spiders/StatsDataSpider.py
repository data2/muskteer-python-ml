...
class StatsDataSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        time.sleep(1)
        self.param["dfwds"] = json.dumps([{"wdcode": "zb", "valuecode": "A0201"}], separators=(',', ':'))
    ...
    def parse_content(self, response):
        ...
        if 200 != response.status:
            print("query not success, return statuscode:%s" % response.status)
            return
        print(response.text)
        ...
        for str in wdnodes:
            wdnode_dict = {}
            wdnode_dict['dbcode'] = 'hgnd'
            wdnode_dict['id'] = str['code']
            wdnode_dict['isParent'] = "false"
            wdnode_dict['name'] = str['name']
            wdnode_dict['pid'] = str['code'][0:len(str['code']) - 2]
            wdnode_dict['wdcode'] = "zb"
            print(wdnode_dict)
            for datastr in datanodes:
                if datastr['code'].find(wdnode_dict['id']) > 0:
                    print("  %s : %d  " % (datastr['code'], datastr['data']['data']))
