...
class EastMoneySpider(scrapy.Spider):
    ...
    start_urls = (
        "http://guba.eastmoney.com/list,601360_0.html",
    )
    proxy_pool = []
    ...
    def parse(self, response):
        ...
        request_url = 'http://guba.eastmoney.com/list,' + str(self.stocknum) + '_' + str(self.page) + '.html'
    ...
    def parse_content(self, response):
        ...
        if 200 != response.status:
            ...
            return
        # print(response.text)
        list1 = response.xpath("//div[@id='articlelistnew']/div/span/a/@title").extract()
        for title in list1:
            print(title)
            self.save_to_json(title)
        self.page += 1
        print("第" + str(self.page) + "页")

    def save_to_json(self, content):
        try:
            with open("360.txt", "a") as file:
